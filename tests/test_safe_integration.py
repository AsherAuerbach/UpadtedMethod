...
class TestProjectIntegration:
    ...
    def test_full_project_structure(self, project_root):
        ...
        expected_structure = {
            ...
            "directories": ["injector", "dll-hook", "scripts", "tests", "logs", "bin", "obj"],
            # Files that should exist
            "files": ["requirements.txt", "README.md", ".gitignore", "pyproject.toml"],
            # Files in injector/
            "injector_files": ["injector.py", "inject.py", "logging_utils.py"],
            # Files in dll-hook/
            "cpp_files": ["dllmain.cpp", "logging.cpp", "logging.h", "pch.cpp", "pch.h", "framework.h"],
            # Files in scripts/
            "script_files": ["build-cpp.ps1"],
        }
    ...
    def test_logging_system_integration(self, project_root):
        ...
        test_logs = ["test_integration.log", "test_security.log", "test_dll.log"]
...
class TestSafeBuildSystem:
    ...
    def test_cpp_source_structure(self, cpp_source_dir):
        ...
        header_requirements = ["namespace SecurityResearch", "namespace Logging", "enum class LogLevel", "LOG_INFO", "LOG_ERROR", "LOG_SECURITY_OP"]
...
class TestSafeProcessOperations:
    ...
    def test_process_monitoring_logic(self, mock_subprocess):
        ...
        mock_subprocess.assert_called_with(["tasklist", "/FO", "CSV", "/NH"], capture_output=True, text=True)
...
class TestRegressionPrevention:
    ...
    def test_build_artifacts_locations(self, project_root):
        ...
        expected_locations = ["bin/Debug/x64", "bin/Release/x64", "obj/Debug/x64", "obj/Release/x64"]

        # At least some build directories should exist if build was run
        build_dirs_exist = any((project_root / location).exists() for location in expected_locations)
    ...
    def test_no_dangerous_operations_in_tests(self):
        ...
        test_indicators = ["pytest" in sys.modules, "test" in __file__.lower(), any("test" in arg.lower() for arg in sys.argv)]
