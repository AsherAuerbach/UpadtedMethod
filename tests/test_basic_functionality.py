...
class TestBasicFunctionality:
    ...
    def test_project_structure_exists(self, project_root):
        ...
        required_dirs = ["injector", "dll-hook", "scripts", "tests"]
...
class TestSafeInjectorFunctionality:
    ...
    def test_injector_class_structure(self):
        ...
        try:
            from injector import DLLInjectionError, DLLInjector, ProcessAccessError
    ...
    def test_build_script_syntax_validation(self, scripts_dir):
        ...
        try:
            ...
            result = subprocess.run(
                ["powershell", "-Command", f"& {{ try {{ Get-Content '{build_script}' | Out-String | Invoke-Expression -ErrorAction Stop; Write-Output 'SYNTAX_OK' }} catch {{ Write-Error $_.Exception.Message }} }}"],
                capture_output=True,
                text=True,
                timeout=30,
            )
